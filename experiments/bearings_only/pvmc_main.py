...
from experiments.common.training_stage import VanillaPydpfRun, TrainingStage, Trainer
...
class masked_loss(pydpf.Module):
    ...
    def forward(self, **data):
        ...
        mask[self.keep_mod - 1::self.keep_mod] = 1
        return temp * mask
...
def make_training_stages(device):
    ...
    first_knll, real_knll = build_output_funcs(device)
    ...
    scale_params = (ParameterSet(SSM.dynamic_model.tolerance_dist)
                    ...
                    + ParameterSet(real_knll))


    stage_one_opt = torch.optim.Adam([{"params": prop_only_param - scale_params + encoder_params, "lr": 1e-3}, {"params": scale_params - ParameterSet(first_knll), "lr": 1e-2}])
    stage_two_opt = torch.optim.Adam([{"params": prop_only_param - scale_params  + encoder_params, "lr": 1e-3}, {"params": scale_params, "lr": 1e-3}])
    stage_three_opt = torch.optim.Adam([{"params": pvmc_param - prop_only_param - scale_params, "lr": 1e-3}, {"params": scale_params, "lr": 1e-3}])
    stage_four_opt = torch.optim.Adam([{"params": pvmc_param + encoder_params, "lr": 1e-4}])
    ...
    info_test = {"n_particles": 50,
                 ...
                 "time_extent": 49,
                 "output_function": {"KNLL": real_knll}}
    ...
    stage_three_info = {"train": { **info_train, "output_function": {"KNLL": masked_loss(4, real_knll) }},
                                        "validation": { **info_validation, "output_function": {"KNLL": real_knll}},
                                        "loss": "4 * time_average.KNLL",
                                        "print_each_epoch": {"train loss": "4 * train.mean.time_average.KNLL", "validation loss": "validation.mean.time_average.KNLL"},
                                        "epochs": 50,
                                        "device": "cuda:0",
                                        "print": {"test loss": "test.mean.time_average.KNLL"},
                                        "test": info_test
                                        }

    stage_four_info = stage_three_info.copy()

    return [stage_one_train, stage_two_train, stage_three_train, stage_four_train], pvmc, encoder, real_knll, first_knll, [stage_one_info, stage_two_info, stage_three_info, stage_four_info]

def make_training_routine(device):
    stages, pvmc, encoder, real_knll, first_knll, info, = make_training_stages(device)

    routine = Trainer(pvmc, encoder, real_knll, first_knll, encoder, stages = stages)
    return routine, info


if __name__ == "__main__":
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    routine, info = make_training_routine(device)
